...
- รู้จัก prefix ของหลายเวที (picoCTF, HTB, THCTT, NCSA, Aegis และอื่นๆ) + generic candidate เช่น `customEvent{...}`
- ถ้าโจทย์ระบุ `prefix=` หรือ `flag_format=` จะใช้ prefix นั้นกับ solver ที่ต้องสร้าง flag; ถ้าไม่ระบุจะคืน plaintext/candidate โดยไม่เดาเป็น `AegisCTF{...}`
