...
MODE="${1:-check}"
TOOLS=(nmap tshark exiftool binwalk p7zip zbar tesseract)
...
if [[ "$(uname -s)" == "Darwin" ]]; then
  ...
  brew update
  brew install nmap wireshark exiftool binwalk p7zip zbar tesseract || true
elif command -v apt-get >/dev/null 2>&1; then
  sudo apt-get update
  sudo apt-get install -y nmap tshark libimage-exiftool-perl binwalk p7zip-full zbar-tools tesseract-ocr
else
